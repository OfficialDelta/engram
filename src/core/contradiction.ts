...
import type { ContradictionResult, ContradictionConfig } from '../types.js';
...
const contradictionTool = {
  ...
  description: 'Evaluate whether a file write contradicts existing project decisions',
  input_schema: {
    type: 'object',
    properties: {
      verdict: {
        type: 'string',
        enum: ['NO_CONTRADICTION', 'INDIRECT_CONTRADICTION', 'DIRECT_CONTRADICTION'],
      },
...
export class ContradictionChecker {
  ...
  async checkContradiction(
    ...
    try {
      const client: HaikuClient = (config?.client as HaikuClient) ??
