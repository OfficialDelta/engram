...
export function computeStrength(params: StrengthParams): number {
	...
	const outcomeConsistency =
		params.totalEpisodes === 0
			? 0.5
			: params.successfulEpisodes / params.totalEpisodes;
	const causalImportance = params.causallyImportant ? 1.5 : 1.0;
	return Math.min(
		Math.max(frequency * recency * outcomeConsistency * causalImportance, 0),
		1,
	);
}
