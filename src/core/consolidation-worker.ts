...
if (isEpisodeComplete(dataDir, sessionId)) {
	fs.writeFileSync(
		logPath,
		`[${new Date().toISOString()}] Skipped: episode already exists for session ${sessionId}\n`,
	);
	process.exit(0);
}
...
if (!acquireConsolidationLock(dataDir, sessionId)) {
	fs.writeFileSync(
		logPath,
		`[${new Date().toISOString()}] Skipped: consolidation lock held for session ${sessionId}\n`,
	);
	process.exit(0);
}
...
if (fs.existsSync(dbPath)) {
	let checkDb: InstanceType<typeof Database> | undefined;
	try {
		...
		if (mismatch) {
			...
			fs.writeFileSync(
				logPath,
				`[${new Date().toISOString()}] Failed: ${errorMsg}\n`,
			);
...
try {
	...
	const consolidationConfig = {
		...(cfg.consolidation?.provider
			? { consolidationProvider: cfg.consolidation.provider }
			: {}),
		...(cfg.llm.pass1Model ? { pass1Model: cfg.llm.pass1Model } : {}),
		...(cfg.llm.pass2Model ? { pass2Model: cfg.llm.pass2Model } : {}),
		...(cfg.consolidation.windowSize
			? { windowSize: cfg.consolidation.windowSize }
			: {}),
		...(cfg.consolidation.windowOverlap
			? { windowOverlap: cfg.consolidation.windowOverlap }
			: {}),
	...
	try {
		...
		/* state reset is best-effort */
	}
	fs.writeFileSync(
		logPath,
		`[${new Date().toISOString()}] Consolidation completed for session ${sessionId}\n`,
	);
	process.exit(0);
} catch (err) {
	releaseConsolidationLock(dataDir, sessionId);
	...
	fs.writeFileSync(
		logPath,
		`[${new Date().toISOString()}] Consolidation failed for session ${sessionId}:\n${message}\n`,
	);
	try {
		const episodesDir = path.join(dataDir, "episodes");
		fs.mkdirSync(episodesDir, { recursive: true });
