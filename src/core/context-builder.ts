...
function isCrossCuttingDecision(nr: NodeResult): boolean {
	return (
		nr.node.nodeType === "decision" &&
		nr.node.affectedFiles.length >= CROSS_CUTTING_THRESHOLD
	);
}
...
function renderGrouped(
	...
	const remainderHigh = tieredResults.high.filter(
		(nr) => !isCrossCuttingDecision(nr),
	);
	const remainderMed = tieredResults.medium.filter(
		(nr) => !isCrossCuttingDecision(nr),
	);
