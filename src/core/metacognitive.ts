...
export function progressVelocity(events: EngramEvent[]): ProgressVelocity {
  if (events.length === 0) {
    return { currentVelocity: 0, trend: 'stable', uniqueFilesWritten: 0, windowSize: 0 };
  }
...
export function searchToActRatio(events: EngramEvent[]): SearchToActRatio {
  ...
  for (const event of events) {
    if (event.type === 'file_read' || event.type === 'research' || event.type === 'exploration') {
      reads++;
  ...
  const isConcerning = ratio > 5.0 && progressFraction > 0.5 && directorySpread > 3;

  return { ratio, isConcerning, reads, writes, progressFraction, directorySpread };
}
...
export function errorRepetition(events: EngramEvent[]): ErrorRepetition {
  ...
  const repeatedErrors: Array<{ file: string; errorType: string; count: number }> = [];
