...
const TEST_PATTERN = /\b(npm\s+test|vitest|jest|node\s+--test|pytest|go\s+test|cargo\s+test)\b/i;
const BUILD_PATTERN = /\b(npm\s+run\s+build|tsc|make\b|cargo\s+build|go\s+build|webpack)\b/i;
...
export function classifyToolCall(toolCall: RawToolCall): EngramEvent | null {
  ...
  if (tool_name === 'Bash') {
    ...
    const exitCode = typeof tool_input.exit_code === 'number' ? tool_input.exit_code : 0;
  ...
  if (tool_name === 'WebSearch' || tool_name === 'WebFetch') {
    return {
      ...
      sessionId,
      timestamp,
      query: (tool_input.query as string) ?? (tool_input.command as string) ?? '',
...
export function detectDerivedEvents(primaryEvent: EngramEvent, priorEvents: EngramEvent[]): EngramEvent[] {
  ...
  if (primaryEvent.type === 'file_read') {
    const priorReadPaths = new Set(
      priorEvents
        .filter((e): e is Extract<EngramEvent, { type: 'file_read' }> => e.type === 'file_read')
        .map(e => e.filePath),
    );
  ...
  if (primaryEvent.type === 'file_write') {
    const mostRecentTestRun = findLastOfType(priorEvents, 'test_run') as TestRunEvent | undefined;
    ...
    const priorWritePaths = priorEvents
      .filter((e): e is Extract<EngramEvent, { type: 'file_write' }> => e.type === 'file_write')
      .map(e => e.filePath);

    if (!priorWritePaths.includes(primaryEvent.filePath) && mostRecentTestRun?.passed) {
      derived.push({
        type: 'progression',
        sessionId,
        timestamp,
    ...
    const priorWriteCount = priorEvents.filter(
      (e): e is Extract<EngramEvent, { type: 'file_write' }> => e.type === 'file_write' && e.filePath === primaryEvent.filePath,
    ).length;
...
function findLastOfType(events: EngramEvent[], type: string): EngramEvent | undefined {
...
function maxDirectoryDistance(filePaths: string[]): number {
  ...
  for (let i = 0; i < segmentSets.length; i++) {
    for (let j = i + 1; j < segmentSets.length; j++) {
      ...
      let commonLen = 0;
      while (commonLen < a.length && commonLen < b.length && a[commonLen] === b[commonLen]) {
        commonLen++;
      }
...
export function appendEvent(sessionId: string, event: EngramEvent, dataDir?: string): void {
  try {
...
export function getSessionEvents(sessionId: string, dataDir?: string): EngramEvent[] {
  const base = dataDir ?? path.join(os.homedir(), '.engram');
