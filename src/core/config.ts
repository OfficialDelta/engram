...
export function loadConfig(overridePath?: string): EngramConfig {
	...
	try {
		...
		if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
			...
			process.stderr.write(
				`engram: config at ${filePath} is not a JSON object, using defaults\n`,
			);
		}
	} catch (err: unknown) {
		if (
			...
			process.stderr.write(
				`engram: failed to read config at ${filePath}, using defaults\n`,
			);
		}
	}
	...
	const config: EngramConfig = {
		...
		...(fileConfig.maintenance
			? { maintenance: { ...fileConfig.maintenance } }
			: {}),
	};
	...
	if (
		...
		(provider === "openai" || provider.startsWith("text-embedding"))
	) {
...
export function validateConfig(config: unknown): {
	...
	if (c.embedding && typeof c.embedding === "object") {
		...
		if (
			...
			typeof emb.provider === "string" &&
			!KNOWN_PROVIDERS.includes(emb.provider)
		) {
			errors.push(
				`Unknown embedding provider "${emb.provider}". Known: ${KNOWN_PROVIDERS.join(", ")}`,
			);
		}
	}
	...
	if (c.consolidation && typeof c.consolidation === "object") {
		...
		if (
			con.provider !== undefined &&
			!["api", "claude-cli"].includes(con.provider as string)
		) {
...
export function getMaintenanceConfig(config: EngramConfig): {
	decayThreshold: number;
	decayFactor: number;
} {
	return {
		decayThreshold:
			config.maintenance?.decayThreshold ?? MAINTENANCE_DEFAULTS.decayThreshold,
		decayFactor:
			config.maintenance?.decayFactor ?? MAINTENANCE_DEFAULTS.decayFactor,
	};
}
