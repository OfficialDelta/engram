...
async function embedLocal(texts: string[]): Promise<number[][]> {
  ...
  if (texts.length === 1 && nested.length === 1 && Array.isArray(nested[0]![0])) {
  ...
  return Promise.all(texts.map(async (text) => {
    ...
    try {
      ...
      if (err instanceof DOMException && err.name === 'AbortError') {
        throw new Error(`Ollama connection timed out at ${ollamaUrl}. Is Ollama running? Start with: ollama serve`);
      }
      const msg = err instanceof Error ? err.message : String(err);
      console.error(`Ollama connection failed: ${msg}`);
      throw new Error(`Cannot reach Ollama at ${ollamaUrl}. Is Ollama running? Start with: ollama serve`);
    }
    clearTimeout(timeout);
...
export async function getEmbedding(
  texts: string[],
  config?: { provider?: string; apiKey?: string; ollamaUrl?: string; ollamaModel?: string },
): Promise<number[][]> {
  ...
  if (provider === 'ollama') {
    const ollamaUrl = config?.ollamaUrl ?? fileConfig.embedding.ollamaUrl ?? 'http://localhost:11434';
