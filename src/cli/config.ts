...
import { loadConfig, saveConfig, maskApiKey, type EngramConfig } from '../core/config.js';
  ...
  for (const [section, values] of Object.entries(config)) {
    if (values && typeof values === 'object') {
      for (const [key, val] of Object.entries(values as Record<string, unknown>)) {
        ...
        const display = KEY_FIELDS.has(dotPath) && typeof val === 'string'
          ? maskApiKey(val)
          : String(val);
...
function configGet(dotPath: string, overridePath?: string): void {
  ...
  const config = loadConfig(overridePath);
  const value = getNestedValue(config as unknown as Record<string, unknown>, dotPath);
  ...
  const display = KEY_FIELDS.has(dotPath) && typeof value === 'string'
    ? maskApiKey(value)
    : String(value);
  console.log(display);
}

function configSet(dotPath: string, rawValue: string, overridePath?: string): void {
  if (!KNOWN_KEYS.has(dotPath)) {
    console.error(`Unknown config key: ${dotPath}`);
  ...
  console.log(`Set ${dotPath} = ${KEY_FIELDS.has(dotPath) && typeof value === 'string' ? maskApiKey(value) : value}`);
}

async function configWizard(overridePath?: string): Promise<void> {
  if (!process.stdin.isTTY) {
    console.log('Non-interactive environment detected. Use engram config set <key> <value> instead.');
    return;
  }
...
export async function runConfig(args: string[], overridePath?: string): Promise<void> {
