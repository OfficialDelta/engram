	...
	for (const sessionId of sessionIds) {
		...
		try {
			...
			const consolidationConfig = {
				...(config.consolidation?.provider
					? { consolidationProvider: config.consolidation.provider }
					: {}),
				...(config.llm.pass1Model ? { pass1Model: config.llm.pass1Model } : {}),
				...(config.llm.pass2Model ? { pass2Model: config.llm.pass2Model } : {}),
				...(config.consolidation.windowSize
					? { windowSize: config.consolidation.windowSize }
					: {}),
				...(config.consolidation.windowOverlap
					? { windowOverlap: config.consolidation.windowOverlap }
					: {}),
