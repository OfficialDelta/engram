...
import { createEdge, createNode, getNode, updateNode } from "../db/graph.js";
...
export async function handleSaveDecision(
	...
	try {
		...
		if (cfg.embedding?.provider)
			embeddingConfig.provider = cfg.embedding.provider;
		...
		if (resolution.action === "merge" && resolution.existingNodeId) {
			...
			if (existing) {
				...
				const existingAlts = Array.isArray(existingMeta.alternatives)
					? (existingMeta.alternatives as string[])
					: [];
				const mergedAlts = [
					...new Set([...existingAlts, ...args.alternatives_considered]),
				];
					...
					content: [
						{
							...
							text: JSON.stringify({
								action: "merged",
								nodeId: resolution.existingNodeId,
							}),
						},
					],
				};
		...
		return {
			content: [
				{
					type: "text",
					text: JSON.stringify({ action: "created", nodeId: newNode.id }),
				},
			],
		};
	} catch (error) {
	...
	server.tool(
		...
		"Query the engram knowledge graph with a natural language question",
		{
			question: z
				.string()
				.describe(
					"Natural language question or context to retrieve knowledge for",
				),
		},
