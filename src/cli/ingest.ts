...
import { resolve, relative, dirname, extname, basename, join } from 'node:path';
...
const IGNORE_DIRS = new Set([
  'node_modules', 'dist', 'build', '.git', '.gsd', '.next',
  '__pycache__', '.venv', 'venv', 'target', 'vendor',
  'coverage', '.cache', 'tmp', '.idea', '.vscode',
]);
...
function walkFiles(root: string): string[] {
  ...
  function walk(dir: string): void {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (CODE_EXTENSIONS.has(ext) || DOC_EXTENSIONS.has(ext) || name === 'package.json') {
  ...
  if (results.length >= MAX_FILES) {
    process.stderr.write(`Warning: file limit (${MAX_FILES}) reached, some files skipped\n`);
  }
...
function resolveImport(importPath: string, fileSet: Set<string>): string | null {
...
function readGitCommits(cwd: string, limit: number): Array<{ hash: string; message: string }> {
  try {
    const output = execSync('git log --oneline --no-decorate -' + String(limit), {
      cwd,
      encoding: 'utf-8',
      timeout: 10000,
    ...
    process.stderr.write('Warning: git not available, skipping commit episodes\n');
    return [];
  }
}
...
export async function runIngest(options: IngestOptions): Promise<IngestResult> {
  ...
  if (!existsSync(dbPath)) {
    ...
    return { filesFound: 0, nodesCreated: 0, edgesCreated: 0, episodesCreated: 0, nodesEmbedded: 0 };
  }
  ...
  if (options.dryRun) {
    ...
    console.log(`Dry run: would create ~${files.length} file nodes, ~${docFiles.length} doc nodes, ${commits.length} episodes`);
    console.log(`Code files: ${codeFiles.length}, Doc files: ${docFiles.length}`);
    return { filesFound: files.length, nodesCreated: 0, edgesCreated: 0, episodesCreated: 0, nodesEmbedded: 0 };
  }
  ...
  db.transaction(() => {
    for (const filePath of files) {
      ...
      allNodeIds.push({ id: node.id, text: node.name + ' ' + node.description });
      nodesCreated++;
    }
  })();
  ...
  if (canEmbed && allNodeIds.length > 0) {
    ...
    try {
      ...
      process.stderr.write(`Warning: embedding failed (${err instanceof Error ? err.message : String(err)}), skipping embedding pass\n`);
    }
  ...
  return { filesFound: files.length, nodesCreated, edgesCreated, episodesCreated, nodesEmbedded };
}
