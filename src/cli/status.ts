...
function getDbStats(dbPath: string): DbStats | { error: string } {
	...
	try {
		...
		const lastRow = db
			.prepare("SELECT MAX(timestamp) AS last FROM episodes")
...
function checkHooksRegistered(settingsPath: string): {
	registered: string[];
	missing: string[];
} {
	...
	for (const event of HOOK_EVENT_NAMES) {
		...
		const matchers: MatcherBlock[] = Array.isArray(raw)
			? raw
			: raw && typeof raw === "object" && "hooks" in raw
				? [{ matcher: "", hooks: (raw as { hooks: HookEntry[] }).hooks }]
				: [];
...
export function runStatus(options: StatusOptions): StatusResult {
	...
	let failedConsolidations: Array<{
		sessionId: string;
		error: string;
		timestamp: string;
	}> = [];
...
export function formatStatus(result: StatusResult): string {
	...
	if (result.failedConsolidations.length > 0) {
		const sorted = [...result.failedConsolidations].sort((a, b) =>
