...
import { Database, initializeSchema } from "../db/migrations.js";
...
export async function runReEmbed(
	...
	try {
		nodes = readDb
			.prepare("SELECT id, name, description FROM nodes")
	...
	if (options.dryRun) {
		console.log(
			`Dry run: would re-embed ${nodeCount} nodes with ${provider} (${dimension}d)`,
		);
	...
	console.log(
		`Re-embed complete: ${nodeCount} nodes, provider=${provider}, dimension=${dimension}`,
	);
	db.close();
