...
export function runInspect(options: InspectOptions): InspectResult {
	...
	const totalEpisodes = (
		db.prepare("SELECT COUNT(*) AS count FROM episodes").get() as {
			count: number;
		}
	).count;

	const nodesByType = db
		.prepare(
	...
	const recentEpisodesRaw = db
		.prepare(
			"SELECT id, session_id, summary, timestamp FROM episodes ORDER BY timestamp DESC LIMIT 5",
		)
		.all() as Array<{
		id: string;
		session_id: string;
	...
	if (options.type) {
		highStrengthRaw = db
			.prepare(
				"SELECT id, name, node_type, strength, description FROM nodes WHERE strength > 0.7 AND node_type = ? ORDER BY strength DESC LIMIT ?",
			)
			.all(options.type, limit) as typeof highStrengthRaw;
		supersededRaw = db
			.prepare(
				"SELECT id, name, node_type, description FROM nodes WHERE strength = 0 AND node_type = ?",
			)
			.all(options.type) as typeof supersededRaw;
	} else {
		highStrengthRaw = db
			.prepare(
				"SELECT id, name, node_type, strength, description FROM nodes WHERE strength > 0.7 ORDER BY strength DESC LIMIT ?",
			)
			.all(limit) as typeof highStrengthRaw;
		supersededRaw = db
			.prepare(
				"SELECT id, name, node_type, description FROM nodes WHERE strength = 0",
			)
			.all() as typeof supersededRaw;
	}
	...
	return {
		...
		edgesByType: edgesByType.map((r) => ({
			type: r.relationship_type,
			count: r.count,
		})),
		recentEpisodes: recentEpisodesRaw.map((r) => ({
			id: r.id,
			sessionId: r.session_id,
			summary: r.summary,
			timestamp: r.timestamp,
		})),
		highStrengthNodes: highStrengthRaw.map((r) => ({
			id: r.id,
			name: r.name,
			type: r.node_type,
			strength: r.strength,
		...
		supersededNodes: supersededRaw.map((r) => ({
			id: r.id,
			name: r.name,
			type: r.node_type,
			description: r.description,
...
export function formatInspect(result: InspectResult): string {
	...
	if (result.highStrengthNodes.length === 0) {
		...
		for (const node of result.highStrengthNodes) {
			lines.push(
				`    ${node.strength} ${node.name} (${node.type}) — ${node.description}`,
			);
		}
	}
