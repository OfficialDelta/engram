...
function _rowToEpisode(row: EpisodeRow): Episode {
	return {
		id: row.id,
...
export function getDecisionNodesForFile(
	db: Database,
	filePath: string,
): GraphNode[] {
	const pattern = `%"${escapeLike(filePath)}"%`;
