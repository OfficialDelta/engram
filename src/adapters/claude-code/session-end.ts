...
function logError(dataDir: string, message: string): void {
  try {
    ...
    appendFileSync(join(logDir, 'session-end.log'), `[${new Date().toISOString()}] ${message}\n`);
  } catch {
...
function main(): void {
  try {
    ...
    try {
      ...
      logError(dataDir, `SessionEnd handler error: ${err instanceof Error ? err.stack ?? err.message : String(err)}`);
    } catch {
      // final fallback
    }
    process.stdout.write('{}');
