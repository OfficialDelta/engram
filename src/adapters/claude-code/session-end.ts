...
export function processSessionEnd(
	...
	if (
		cfg.consolidation?.provider !== "claude-cli" &&
		!cfg.llm.apiKey &&
		!process.env.ANTHROPIC_API_KEY
	) {
		logError(
			dataDir,
			'Consolidation skipped: no API key configured. Run "engram config" to set up.',
		);
		return {};
	}

	const events = getSessionEvents(sessionId, dataDir);
	const lastTimestamp = readConsolidationTimestamp(dataDir, sessionId);

	const unconsolidated = lastTimestamp
		? events.filter((e) => e.timestamp > lastTimestamp)
		: events;
	...
	if (state.consolidationSpawned) {
		logError(
			dataDir,
			`Consolidation spawn skipped: already spawned for session ${sessionId}`,
		);
		return {};
	}
...
function main(): void {
	try {
		...
		processSessionEnd(sessionId, dataDir, dbPath);
		...
		try {
			...
			logError(
				dataDir,
				`SessionEnd handler error: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`,
			);
		} catch {
			// final fallback
		}
