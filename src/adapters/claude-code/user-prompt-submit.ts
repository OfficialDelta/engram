...
async function main(): Promise<void> {
	try {
		...
		try {
			...
			logError(
				dataDir,
				`Spreading activation failed: ${err instanceof Error ? err.message : String(err)}`,
			);
		}
		...
		try {
			...
			logError(
				dataDir,
				`UserPromptSubmit handler error: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`,
			);
		} catch {
			// final fallback
		}
