...
export function processStop(
  ...
  const turnEvent = buildTurnCompleteEvent(sessionId, state.toolCallCount, state.turnCount, {
    ...
    ...(options?.agentSummary !== undefined ? { agentSummary: options.agentSummary } : {}),
  });
...
function main(): void {
  try {
    ...
    const conversation = (input.conversation ?? input.messages) as Array<{ role: string; content: string }> | undefined;
    ...
    processStop(sessionId, dataDir, dbPath, agentSummary !== undefined ? { agentSummary } : undefined);
