...
export function spawnContradictionCheck(
	...
	try {
		const workerPath = join(
			...
			"contradiction-worker.js",
		);
		const child = spawn(
			process.execPath,
			[workerPath, sessionId, dbPath, dataDir, filePath, evidenceSnippet],
			{
				detached: true,
				stdio: "ignore",
			},
		...
		logError(
			dataDir,
			`Failed to spawn contradiction worker: ${err instanceof Error ? err.message : String(err)}`,
		);
	}
}
...
export function processPostToolUse(
	...
	if (event.type === "file_read") {
		...
		if (!state.seenFiles.includes(readEvent.filePath)) {
			try {
				...
				logError(
					dataDir,
					`getFileAnnotations failed: ${err instanceof Error ? err.message : String(err)}`,
				);
			}
	...
	const additionalContext = buildContext(pendingContradictions, annotations, {
		high: [],
		medium: [],
	});
...
function main(): void {
	try {
		...
		try {
			...
			logError(
				dataDir,
				`PostToolUse handler error: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`,
			);
		} catch {
