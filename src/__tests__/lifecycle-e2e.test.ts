...
const batch1Pass2Input: { episode: StructuredEpisode; changes: GraphChangeRequest } = {
  episode: {
...
const batch2Pass1JSON = JSON.stringify({
  ...
  decisionsIdentified: ['Structured logging format', 'Centralized error handler'],
  outcome: 'progress',
});

const batch2Pass2Input: { episode: StructuredEpisode; changes: GraphChangeRequest } = {
  episode: {
    goal: 'Configure logging',
    approach: 'Set up structured logging and centralized error handling',
    outcome: 'success',
    discoveries: [{ content: 'Structured logs improve debugging', evidence: 'src/config.ts:30', confidence: 0.85 }],
    decisions: [{ content: 'Use centralized error handler', rationale: 'Reduces duplication', isImplicit: false }],
    errors: [],
  },
  changes: {
    nodesToCreate: [
...
describe('Full Lifecycle with PostCompact Consolidation Cycle', () => {
  ...
  it('full lifecycle: SessionStart through SessionEnd with two consolidation cycles', async () => {
    ...
    const { consolidateSession, readConsolidationTimestamp, writeConsolidationTimestamp, spawnConsolidation } =
      await import('../core/consolidation.js');
    const { getSessionEvents } = await import('../core/event-stream.js');
    const { loadSessionState, saveSessionState } = await import('../core/session-state.js');
    const { initializeSchema } = await import('../db/migrations.js');
    ...
    processPostToolUse(
      { tool_name: 'Read', tool_input: { file_path: 'src/auth.ts' }, session_id: sessionId },
      tmpDir,
      dbPath,
      { spawnCheck: vi.fn() },
    );
    ...
    processPostToolUse(
      { tool_name: 'Write', tool_input: { file_path: 'src/auth.ts', content: 'jwt middleware code' }, session_id: sessionId },
      tmpDir,
      dbPath,
      { spawnCheck: vi.fn() },
    );
    ...
    const rows1 = db1.prepare('SELECT name FROM nodes').all() as Array<{ name: string }>;
    ...
    processPostToolUse(
      { tool_name: 'Read', tool_input: { file_path: 'src/config.ts' }, session_id: sessionId },
      tmpDir,
      dbPath,
      { spawnCheck: vi.fn() },
    );
    ...
    processPostToolUse(
      { tool_name: 'Write', tool_input: { file_path: 'src/config.ts', content: 'logging config' }, session_id: sessionId },
      tmpDir,
      dbPath,
      { spawnCheck: vi.fn() },
    );
    ...
    expect(vi.mocked(spawnConsolidation).mock.calls.length).toBeGreaterThan(spawnCallsBefore);
    ...
    await consolidateSession(sessionId, dbPath, tmpDir, { client: mockClient2, sinceTimestamp: ts! });
    writeConsolidationTimestamp(tmpDir, sessionId, new Date().toISOString());

    // Verify all 4 nodes exist in DB
    const db2 = initializeSchema(dbPath);
    const rows2 = db2.prepare('SELECT name FROM nodes').all() as Array<{ name: string }>;
  ...
  it('SessionEnd skips when no unconsolidated events remain', async () => {
    ...
    const { processStop } = await import('../adapters/claude-code/stop.js');
    const { processSessionEnd } = await import('../adapters/claude-code/session-end.js');
    const { consolidateSession, writeConsolidationTimestamp, spawnConsolidation } =
      await import('../core/consolidation.js');
    ...
    processSessionEnd(sessionId, tmpDir, dbPath);
    expect(vi.mocked(spawnConsolidation).mock.calls.length).toBe(callCountBefore);
  });
