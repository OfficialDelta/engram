...
vi.mock("../core/project-identity.js", async (importOriginal) => {
	const orig =
		await importOriginal<typeof import("../core/project-identity.js")>();
	return {
		...orig,
...
import type {
	GSDActiveUnit,
	GSDBeforeAgentStartEvent,
	GSDExtensionAPI,
	GSDExtensionContext,
	GSDToolCallEvent,
} from "../adapters/gsd/types.js";
...
function createMockGSDAPI() {
	...
	return {
		...
		trigger(eventName: keyof HandlerMap, eventData?: unknown) {
			...
			return (list[0] as (event: unknown, ctx: GSDExtensionContext) => unknown)(
				eventData,
				{},
			);
		},
		setPhase(p: string | null) {
			phase = p;
		},
		setActiveUnit(u: GSDActiveUnit | null) {
			activeUnit = u;
		},
	};
}
...
function readAllEvents(): EngramEvent[] {
	...
	return content
		...
		.split("\n")
		.filter(Boolean)
		.map((l) => JSON.parse(l) as EngramEvent);
}
...
describe("GSD adapter: tool name/input mapping", () => {
	...
	it("maps read tool with path→file_path normalization", async () => {
		...
		expect((readEvent as { filePath: string }).filePath).toBe(
	...
	it("maps edit tool with oldText→old_string and newText→new_string normalization", async () => {
		...
		expect((writeEvent as { filePath: string }).filePath).toBe(
	...
	it("injects retrieval context into system prompt when graph has connected nodes", async () => {
		...
		if (result && typeof result === "object" && "systemPrompt" in result) {
			expect((result as { systemPrompt: string }).systemPrompt).toContain(
				"You are helping with src/auth.ts",
			);
			expect((result as { systemPrompt: string }).systemPrompt).toContain(
				"TokenStore",
			);
