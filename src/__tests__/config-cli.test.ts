...
import { loadConfig, type EngramConfig } from '../core/config.js';
...
describe('config show', () => {
  it('outputs config with masked keys', async () => {
    const config: EngramConfig = {
      llm: { apiKey: 'sk-1234567890abcdef', pass1Model: 'claude-sonnet-4-20250514' },
...
describe('config set', () => {
  ...
  it('rejects unknown key paths', async () => {
    ...
    await expect(runConfig(['set', 'foo.bar', 'baz'], configPath)).rejects.toThrow('exit');
    expect(consoleErrors.some(s => s.includes('Unknown config key'))).toBe(true);
    exitSpy.mockRestore();
  });
...
describe('config wizard (non-TTY)', () => {
  it('detects non-TTY and skips gracefully', async () => {
    ...
    expect(consoleLogs.some(s => s.includes('Non-interactive environment'))).toBe(true);
  });
});
