...
vi.mock("../db/migrations.js", () => ({
	// biome-ignore lint/complexity/useArrowFunction: must be a regular function for `new` constructor calls
...
describe("runInspect", () => {
	...
	it("passes correct LIMIT for --top N", async () => {
		...
		const limitCall = allCalls.find(
			(c: unknown[]) => c.length > 0 && c[c.length - 1] === 5,
		);
	...
	it("filters queries by --type", async () => {
		...
		const allCallsWithConcept = mockAll.mock.calls.filter((c: unknown[]) =>
			c.includes("concept"),
		);
	...
	it("maps DB column names to result interface fields", async () => {
		...
		mockAll.mockImplementation(() => {
			...
			if (allCallCount === 2)
				return [{ relationship_type: "relates_to", count: 5 }];
			if (allCallCount === 3)
				return [
					{
						id: "ep1",
		...
		expect(result.highStrengthNodes[0]).toEqual({
			id: "n1",
			name: "TestNode",
			type: "concept",
			strength: 0.9,
		...
		expect(result.supersededNodes[0]).toEqual({
			id: "n2",
			name: "OldNode",
			type: "decision",
			description: "Superseded decision",
...
describe("formatInspect", () => {
	it("produces readable text with all sections", async () => {
			...
			supersededNodes: [
				{
					...
					name: "OldAuth",
					type: "decision",
					description: "Legacy auth approach",
				},
			],
		};
		...
		expect(output).toContain(
			"0.95 AuthModule (concept) — Authentication module",
		);
