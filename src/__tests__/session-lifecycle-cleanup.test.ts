...
vi.mock('../core/config.js', () => ({
  ...
  getMaintenanceConfig: vi.fn(() => ({ decayThreshold: 0.05, decayFactor: 0.9 })),
}));

function createTmpDir(): string {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'engram-lifecycle-cleanup-'));
...
describe('SessionStart injected.json reset', () => {
  ...
  it('creates empty injected.json on fresh session', async () => {
    ...
    const injectedPath = path.join(tmpDir, 'events', 'sess-fresh.injected.json');
  ...
  it('resets injected.json from prior session data', async () => {
    ...
    const injectedPath = path.join(tmpDir, 'events', 'sess-reset.injected.json');
...
describe('Dedup filtering logic', () => {
  ...
  function writeInjected(dir: string, sessionId: string, ids: string[]): void {
    fs.writeFileSync(path.join(dir, 'events', `${sessionId}.injected.json`), JSON.stringify(ids));
  }

  function applyDedupFilter(
    tieredResults: { high: Array<{ node: { id: string } }>; medium: Array<{ node: { id: string } }> },
    injectedIds: string[],
  ): {
    filtered: { high: Array<{ node: { id: string } }>; medium: Array<{ node: { id: string } }> };
    newIds: string[];
  } {
  ...
  it('second filter excludes first filter nodes', () => {
    ...
    const { filtered: secondFiltered } = applyDedupFilter(secondResults, second);
