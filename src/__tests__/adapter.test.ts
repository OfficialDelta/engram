...
describe('context-builder', () => {
  const makeContradiction = (overrides?: Partial<ContradictionResult>): ContradictionResult => ({
    verdict: 'DIRECT_CONTRADICTION',
    severity: 'high',
  ...
  const makeAnnotation = (name: string, desc: string): Annotation => ({
    nodeId: 'n1', name, description: desc, strength: 0.9,
  });

  const makeNode = (name: string, desc: string): GraphNode => ({
    id: 'n1', name, nodeType: 'concept', description: desc,
    affectedFiles: [], strength: 1.0, metadata: {},
    createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
  });

  const makeNodeResult = (name: string, desc: string, activation: number): NodeResult => ({
  ...
  it('formats annotations with knowledge prefix', () => {
    const result = buildContext([], [makeAnnotation('Auth', 'Authentication module')], emptyTiered);
  ...
  it('truncates activation results with small budget but preserves contradictions and annotations', () => {
    ...
    const result = buildContext([contradiction], [annotation], tiered, tightBudget);
...
describe('PostToolUse handler', () => {
  ...
  it('file_read event adds to seenFiles and returns annotations', () => {
    ...
    vi.mocked(getFileAnnotations).mockReturnValueOnce([
      { nodeId: 'n1', name: 'AuthModule', description: 'Auth logic', strength: 0.9 },
    ]);

    saveSessionState(dir, 'sess-1', defaultState());

    const result = processPostToolUse(
      { tool_name: 'Read', tool_input: { file_path: '/src/foo.ts' }, session_id: 'sess-1' },
  ...
  it('file_write event triggers contradiction worker spawn', () => {
    const event: FileWriteEvent = {
      type: 'file_write', filePath: '/src/bar.ts',
      linesChanged: 5, evidenceSnippet: 'new code',
      sessionId: 'sess-1', timestamp: new Date().toISOString(),
    };
    vi.mocked(classifyToolCall).mockReturnValueOnce(event);
    saveSessionState(dir, 'sess-1', defaultState());
    ...
    expect(mockSpawn).toHaveBeenCalledWith(
      'sess-1', path.join(dir, 'engram.db'), dir,
      '/src/bar.ts', 'new code',
    );
  });
  ...
  it('pending contradictions consumed and cleared from state', () => {
    ...
    saveSessionState(dir, 'sess-1', {
      ...
      pendingContradictions: [{
        verdict: 'DIRECT_CONTRADICTION', severity: 'high',
        explanation: 'Stale pattern', recommendation: 'Update',
      }],
    });
    ...
    const output = result as { hookSpecificOutput: { additionalContext: string } };
    expect(output.hookSpecificOutput.additionalContext).toContain('CONTRADICTION');
    expect(output.hookSpecificOutput.additionalContext).toContain('Stale pattern');
...
describe('SessionStart handler', () => {
  ...
  it('calls findUnconsolidatedSessions and spawns consolidation', () => {
    vi.mocked(findUnconsolidatedSessions).mockReturnValueOnce(['old-1', 'old-2']);
...
describe('UserPromptSubmit handler', () => {
  it('extracts file paths from prompt text', () => {
    const result = extractEntryPoints('look at ./src/foo.ts and src/bar/baz.ts');
    expect(result.some(e => e.type === 'file' && e.value === './src/foo.ts')).toBe(true);
    expect(result.some(e => e.type === 'file' && e.value === 'src/bar/baz.ts')).toBe(true);
  });
...
describe('Stop handler', () => {
  ...
  it('increments turnCount and appends turn_complete event', () => {
    saveSessionState(dir, 'sess-1', { ...defaultState(), turnCount: 2, toolCallCount: 5 });
