...
import type { CreateNodeInput, ContradictionVerdict } from '../types.js';

function freshDb() {
  return initializeSchema(':memory:');
}

function makeDecisionNode(overrides?: Partial<CreateNodeInput>): CreateNodeInput {
  return {
    name: 'use-typescript',
    nodeType: 'decision',
...
function createMockClient(input: Record<string, unknown>) {
  return {
    messages: {
      create: async () => ({
        content: [{ type: 'tool_use', id: 'test', name: 'check_contradiction', input }],
      }),
    },
  };
}
...
describe('ContradictionChecker', () => {
  it('returns null when no decision nodes found for file', async () => {
    ...
    const result = await checker.checkContradiction(db, 'src/unknown.ts', 'some code', {
      client: createMockClient({ verdict: 'DIRECT_CONTRADICTION', severity: 'high', explanation: 'x', recommendation: 'y' }),
    });
  ...
  it('returns null when all decisions are below strength threshold', async () => {
    ...
    const checker = new ContradictionChecker();
    const result = await checker.checkContradiction(db, 'src/main.ts', 'some code', {
      client: createMockClient({ verdict: 'DIRECT_CONTRADICTION', severity: 'high', explanation: 'x', recommendation: 'y' }),
    });
    expect(result).toBeNull();
  });
  ...
  it('detects direct contradiction', async () => {
    ...
    const result = await checker.checkContradiction(db, 'src/main.ts', 'const x = 1', {
      client: createMockClient(mockResult as unknown as Record<string, unknown>),
    });
  ...
  it('returns null for no contradiction verdict', async () => {
    ...
    createNode(db, makeDecisionNode());
    const checker = new ContradictionChecker();
    const result = await checker.checkContradiction(db, 'src/main.ts', 'good code', {
  ...
  it('disables after 3 consecutive failures', async () => {
    ...
    const result = await checker.checkContradiction(db, 'src/main.ts', 'code', config);
    expect(result).toBeNull();
  });
