...
describe("Full Circuit: Events → Consolidation → Graph → Retrieval → Context", () => {
	...
	it("events → consolidation → graph nodes → spreading activation → buildContext", async () => {
		...
		await consolidateSession("sess-e2e-1", dbPath, tmpDir, {
			client: mockClient,
		});
					...
					content: [
						{
							...
							input: {
								...
								severity: "high",
								explanation:
									"Switching to session tokens directly contradicts the JWT decision",
...
describe("Full Pipeline End-to-End", () => {
	...
	it("full circuit: events → consolidation → graph → retrieval → context → contradiction", async () => {
		...
		const mockClient = createMockClient(pass1JSON, pass2Input);
		await consolidateSession("sess-full", dbPath, tmpDir, {
			client: mockClient,
		});
					...
					content: [
						{
							...
							input: {
								...
								severity: "high",
								explanation:
									"Switching to session tokens contradicts JWT decision",
