...
const { mockExecFileAsync } = vi.hoisted(() => ({
  mockExecFileAsync: vi.fn<(cmd: string, args: string[]) => Promise<{ stdout: string; stderr: string }>>(),
}));
...
describe('parseCliResponse', () => {
  ...
  it('parses valid JSON envelope and returns result', () => {
    expect(parseCliResponse('{"type":"result","result":"hello"}')).toBe('hello');
  });

  it('handles nested JSON in result field', () => {
    expect(parseCliResponse('{"type":"result","result":"{\\"key\\":\\"value\\"}"}')).toBe('{"key":"value"}');
  });
...
describe('invokeClaude', () => {
  ...
  it('calls claude with correct flags in exact order', async () => {
    mockExecFileAsync.mockResolvedValueOnce({ stdout: '{"type":"result","result":"ok"}', stderr: '' });
  ...
  it('throws on non-zero exit code with stderr in error message', async () => {
    mockExecFileAsync.mockRejectedValueOnce({ code: 1, stderr: 'something went wrong' });
...
describe('cliPass1Summarize', () => {
  ...
  it('produces WindowSummary array from JSON response', async () => {
    ...
    mockExecFileAsync.mockResolvedValueOnce({ stdout: cliEnvelope(json), stderr: '' });

    const events: EngramEvent[] = [
      { type: 'file_read', sessionId: 's1', timestamp: '2026-01-01T00:00:00Z', filePath: '/src/a.ts' },
    ];
  ...
  it('handles JSON parse failure gracefully (raw text as summary)', async () => {
    mockExecFileAsync.mockResolvedValueOnce({ stdout: 'not json at all', stderr: '' });
  ...
  it('processes windows sequentially (invokeClaude called N times for N windows)', async () => {
    const json = JSON.stringify({ summary: 's', filesModified: [], decisionsIdentified: [], outcome: 'progress' });
    mockExecFileAsync.mockResolvedValue({ stdout: cliEnvelope(json), stderr: '' });

    const events: EngramEvent[] = [
      { type: 'file_read', sessionId: 's1', timestamp: '2026-01-01T00:00:00Z', filePath: '/src/a.ts' },
    ];
    await cliPass1Summarize([[...events], [...events], [...events]], 'claude-sonnet-4-6');
...
describe('cliPass2Extract', () => {
  ...
  const sampleSummaries: WindowSummary[] = [{
    windowIndex: 0, eventRange: { start: 0, end: 1 },
    summary: 'Agent worked', filesModified: ['/src/a.ts'],
    decisionsIdentified: [], outcome: 'progress',
  }];

  it('produces episode + changes from JSON response', async () => {
    const pass2 = {
      episode: {
        goal: 'Implement feature', approach: 'Read and write files', outcome: 'success',
        discoveries: [], decisions: [], errors: [],
      },
      changes: {
        nodesToCreate: [
          { name: 'Feature', nodeType: 'concept', description: 'desc', affectedFiles: [], causallyImportant: true },
        ],
        nodesToUpdate: [], edgesToCreate: [],
      },
    };
    mockExecFileAsync.mockResolvedValueOnce({
      stdout: `{"type":"result","result":${JSON.stringify(JSON.stringify(pass2))}}`, stderr: '',
    });
  ...
  it('filters nodesToCreate through VALID_NODE_TYPES', async () => {
    const pass2 = {
      episode: { goal: 'G', approach: 'A', outcome: 'success', discoveries: [], decisions: [], errors: [] },
      changes: {
        nodesToCreate: [
          { name: 'Valid', nodeType: 'concept', description: 'd', affectedFiles: [], causallyImportant: false },
          { name: 'Invalid', nodeType: 'unknown_type', description: 'd', affectedFiles: [], causallyImportant: false },
        ],
        nodesToUpdate: [], edgesToCreate: [],
      },
    };
  ...
  it('throws on JSON parse failure (no graceful degradation for Pass 2)', async () => {
    mockExecFileAsync.mockResolvedValueOnce({ stdout: 'not valid json', stderr: '' });

    await expect(cliPass2Extract(sampleSummaries, 'claude-opus-4-6')).rejects.toThrow();
  });
});
...
describe('cliDiscussionConsolidate', () => {
  ...
  it('produces topics/decisions/constraints from JSON response', async () => {
    const disc = { topics: ['testing', 'deployment'], decisions: ['use vitest'], constraints: ['must run in CI'] };
    mockExecFileAsync.mockResolvedValueOnce({
      stdout: `{"type":"result","result":${JSON.stringify(JSON.stringify(disc))}}`, stderr: '',
    });

    const events: EngramEvent[] = [{
      type: 'turn_complete', sessionId: 's1', timestamp: '2026-01-01T00:00:00Z',
      toolCallCount: 0, turnNumber: 1, userMessage: 'hello', agentSummary: 'hi',
    }];
...
describe('hook gating with CLI provider', () => {
  ...
  it('SessionEnd with CLI provider and no API key: should NOT skip consolidation', async () => {
    ...
    vi.mocked(getSessionEvents).mockReturnValue([
      { type: 'file_read', timestamp: '2026-01-02T00:00:00Z', sessionId: 'sess-1', filePath: '/f' },
    ]);

    const { spawnConsolidation } = await import('../core/consolidation.js');
    const { processSessionEnd } = await import('../adapters/claude-code/session-end.js');
...
describe('config validation', () => {
  ...
  beforeEach(async () => {
    const mod = await vi.importActual<typeof import('../core/config.js')>('../core/config.js');
  ...
  it("accepts provider: 'claude-cli'", () => {
    const result = validateConfig({ consolidation: { provider: 'claude-cli' } });
    expect(result.valid).toBe(true);
  });
