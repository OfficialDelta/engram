...
describe('processPostCompact', () => {
  ...
  it('calls spawnConsolidation when API key is present', async () => {
    ...
    const result = processPostCompact('sess-1', tmpDir, path.join(tmpDir, 'engram.db'));

    expect(spawnConsolidation).toHaveBeenCalledWith('sess-1', path.join(tmpDir, 'engram.db'), tmpDir);
...
describe('processSessionEnd', () => {
  ...
  it('spawns consolidation when unconsolidated events exist after timestamp', async () => {
    ...
    (getSessionEvents as ReturnType<typeof vi.fn>).mockReturnValue([
      { type: 'file_read', timestamp: '2026-01-01T00:00:00Z', sessionId: 'sess-1' },
      { type: 'file_read', timestamp: '2026-01-02T00:00:00Z', sessionId: 'sess-1' },
    ]);

    const { readConsolidationTimestamp } = await import('../core/consolidation.js');
    (readConsolidationTimestamp as ReturnType<typeof vi.fn>).mockReturnValue('2026-01-01T00:00:00Z');
  ...
  it('skips consolidation when all events are before timestamp', async () => {
    ...
    const { readConsolidationTimestamp } = await import('../core/consolidation.js');
    (readConsolidationTimestamp as ReturnType<typeof vi.fn>).mockReturnValue('2026-01-02T00:00:00Z');
  ...
  it('consolidates all events when no prior timestamp exists', async () => {
    ...
    const { readConsolidationTimestamp } = await import('../core/consolidation.js');
    (readConsolidationTimestamp as ReturnType<typeof vi.fn>).mockReturnValue(null);
...
describe('readConsolidationTimestamp (real implementation)', () => {
  ...
  it('returns timestamp when valid marker file exists', async () => {
    const mod = await vi.importActual<typeof import('../core/consolidation.js')>('../core/consolidation.js');
    const markerPath = path.join(tmpDir, 'sessions', 'sess-1.last-consolidated-at.json');
    fs.writeFileSync(markerPath, JSON.stringify({ timestamp: '2026-01-15T12:00:00Z' }));
  ...
  it('returns null on malformed JSON', async () => {
    const mod = await vi.importActual<typeof import('../core/consolidation.js')>('../core/consolidation.js');
    const markerPath = path.join(tmpDir, 'sessions', 'sess-bad.last-consolidated-at.json');
...
describe('writeConsolidationTimestamp (real implementation)', () => {
  it('writes correct JSON to the right path', async () => {
    ...
    const content = JSON.parse(fs.readFileSync(markerPath, 'utf-8')) as { timestamp: string };
  ...
  it('creates sessions directory if missing', async () => {
    ...
    try {
      const mod = await vi.importActual<typeof import('../core/consolidation.js')>('../core/consolidation.js');
      mod.writeConsolidationTimestamp(freshDir, 'sess-1', '2026-03-01T00:00:00Z');

      const markerPath = path.join(freshDir, 'sessions', 'sess-1.last-consolidated-at.json');
      expect(fs.existsSync(markerPath)).toBe(true);
    } finally {
