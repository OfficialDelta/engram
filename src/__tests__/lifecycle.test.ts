...
describe("Capture → Consolidate → Retrieve", () => {
	...
	it("events are captured to JSONL via appendEvent", async () => {
		const { appendEvent, getSessionEvents } = await import(
		...
		appendEvent(
			"sess-1",
			{
				...
				filePath: "src/auth.ts",
				linesChanged: 10,
				evidenceSnippet: "jwt middleware",
			},
			tmpDir,
		);
		appendEvent(
	...
	it("consolidateSession populates graph with nodes, edges, and episode marker", async () => {
		...
		expect(
			fs.existsSync(path.join(tmpDir, "episodes", "sess-1.episode.json")),
		).toBe(true);
		...
		const localPass2: typeof pass2Input = {
			...
			changes: {
				...
				nodesToCreate: [
					...
					{
						...pass2Input.changes.nodesToCreate[1]!,
						affectedFiles: [absCrypto],
					},
				],
			},
		};
...
describe("Contradiction Detection Path", () => {
	...
	it("file_write to a file with decisions spawns contradiction check", async () => {
		...
		processPostToolUse(
			{
				tool_name: "Write",
				tool_input: { file_path: "src/auth.ts", content: "new auth code" },
				session_id: "sess-c1",
			},
			tmpDir,
			dbPath,
	...
	it("pending contradiction in session state appears in additionalContext", async () => {
		...
		expect(ctx.hookSpecificOutput!.additionalContext).toContain(
			"⚠️ CONTRADICTION",
		);
...
describe("Crash Recovery", () => {
	...
	it("findUnconsolidatedSessions detects session without episode marker", async () => {
		const { appendEvent } = await import("../core/event-stream.js");
		const { findUnconsolidatedSessions } = await import(
	...
	it("consolidateSession creates episode marker and recovery completes", async () => {
		const { appendEvent } = await import("../core/event-stream.js");
		const { consolidateSession, findUnconsolidatedSessions } = await import(
			"../core/consolidation.js"
		);
		...
		const mockClient = createMockClient(pass1JSON, pass2Input);
		await consolidateSession("sess-recover", dbPath, tmpDir, {
			client: mockClient,
		});

		expect(
			fs.existsSync(path.join(tmpDir, "episodes", "sess-recover.episode.json")),
		).toBe(true);
...
describe("Hook Fast Path Latency (R009)", () => {
	...
	it("processPostToolUse fast path completes under 100ms for file_read", async () => {
		...
		processPostToolUse(
			{
				tool_name: "Read",
				tool_input: { file_path: "src/warmup.ts" },
