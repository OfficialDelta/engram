...
describe("maintenance integration", () => {
	...
	it("full maintenance pass: decay + pattern scan + supersession in single transaction", () => {
		...
		try {
			...
			const weakNode = createNode(
				db,
				makeNode({ name: "weak", strength: 0.04 }),
			);
			// A strong node that will just decay
			const strongNode = createNode(
				db,
				makeNode({ name: "strong", strength: 0.9 }),
			);
			...
			const patternNode = db
				.prepare("SELECT name FROM nodes WHERE node_type = 'pattern'")
	...
	it("all three phases run inside the same transaction (rollback on error)", () => {
		...
		try {
			...
			db.prepare(
				"UPDATE metadata SET value = ? WHERE key = 'last_maintenance_run'",
			).run(old);
			...
			expect(new Date(record.timestamp).getTime()).toBeLessThan(
