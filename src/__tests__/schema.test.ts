...
import { initializeSchema, getSchemaVersion, Database } from '../db/migrations.js';
...
describe('schema initialization', () => {
  ...
  it('seeds metadata rows', () => {
    ...
    const dimension = db.prepare("SELECT value FROM metadata WHERE key = 'embedding_dimension'").get() as { value: string };
  ...
  it('supports concurrent WAL read/write access', () => {
    ...
    const row = reader.prepare("SELECT id, name FROM nodes WHERE id = ?").get('n1') as { id: string; name: string };
  ...
  it('rolls back transaction on error — no partial data committed', () => {
    ...
    const row = db.prepare("SELECT id FROM nodes WHERE id = ?").get('n-partial');
