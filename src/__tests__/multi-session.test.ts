				...
				nodesToCreate: [
					{
						name: "decay-target",
						nodeType: "concept",
						description: "Node to test strength decay",
						affectedFiles: ["src/decay.ts"],
						causallyImportant: false,
					},
				],
				nodesToUpdate: [],
	...
	describe("4. Supersession", () => {
		...
		it("zeroes strength when all affected files are deleted", async () => {
			...
			await simulateSession(db, "sess-supersede", {
				nodesToCreate: [
					{
						name: "superseded-node",
						nodeType: "concept",
						description: "Node with temp files",
	...
	describe("7. Cross-cutting Decision Dedup", () => {
		it("renders cross-cutting decisions in Project-wide decisions section, not in regular Decisions", async () => {
			await simulateSession(db, "sess-cross-1", {
				nodesToCreate: [
					{
						name: "cross-cutting-arch",
						nodeType: "decision",
						description: "Use microservices architecture for all services",
						affectedFiles: ["src/a.ts", "src/b.ts", "src/c.ts"],
						causallyImportant: false,
					},
					{
						name: "local-auth-decision",
						nodeType: "decision",
					...
					},
					{
						name: "some-pattern",
						nodeType: "pattern",
						description: "Singleton pattern for DB connection",
						affectedFiles: ["src/db.ts", "src/app.ts", "src/server.ts"],
						causallyImportant: false,
					},
				],
				nodesToUpdate: [],
			...
			const projectWideSection = sections.find((s) =>
				s.startsWith("Project-wide decisions"),
			);
