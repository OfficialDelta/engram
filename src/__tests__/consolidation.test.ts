...
function _mockEmbedding(): number[] {
...
const defaultPass2Input: {
	...
	episode: {
		...
		outcome: "success",
		discoveries: [
			{ content: "Found pattern", evidence: "file.ts:10", confidence: 0.9 },
		],
		decisions: [
			{ content: "Used approach A", rationale: "Simpler", isImplicit: false },
		],
		errors: [],
	},
	changes: {
...
describe("pass2Extract", () => {
	...
	it("throws when no tool_use block in response", async () => {
		...
		await expect(
			pass2Extract(summaries, client, "claude-opus-4-6"),
		).rejects.toThrow("Pass 2: no tool_use block in response");
	});
...
describe("applyGraphChanges", () => {
	...
	it("new node with success outcome has strength 1.0", async () => {
		...
		try {
			...
			const row = db
				.prepare("SELECT strength FROM nodes WHERE id = ?")
				.get(nodeId) as { strength: number };
	...
	it("merge with failure outcome tracks successfulEpisodeCount and lowers strength", async () => {
		...
		try {
			...
			const map1 = await applyGraphChanges(
				db,
				changes1,
				"sess-1",
				"ep-1",
			...
			const origStrength = (
				db
					.prepare("SELECT strength FROM nodes WHERE id = ?")
					.get(existingId) as { strength: number }
			).strength;
			...
			const row = db
				.prepare("SELECT strength, metadata FROM nodes WHERE id = ?")
				.get(existingId) as { strength: number; metadata: string };
