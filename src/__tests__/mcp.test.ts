...
import { handleQueryKnowledge, handleSaveDecision, runMcp } from '../cli/mcp.js';
import { initializeSchema } from '../db/migrations.js';
import { createNode, createEdge, getNode } from '../db/graph.js';
...
describe('handleSaveDecision', () => {
  ...
  it('returns merged action for duplicate decision', async () => {
    ...
    const existing = createNode(db, {
      ...
      affectedFiles: [],
      strength: 0.8,
      metadata: { rationale: 'Original rationale', alternatives: ['SQLite'], source: 'explicit' },
    });

    mockResolve.mockResolvedValueOnce({ action: 'merge', existingNodeId: existing.id, similarity: 0.95 });
    ...
    expect(meta.alternatives).toEqual(expect.arrayContaining(['SQLite', 'MySQL']));
  });
  ...
  it('creates version_of edge for create_child resolution', async () => {
    ...
    mockResolve.mockResolvedValueOnce({ action: 'create_child', existingNodeId: parent.id, similarity: 0.7 });
    ...
    const edges = db.prepare('SELECT * FROM edges WHERE source_node_id = ? AND relationship_type = ?')
      .all(parsed.nodeId, 'version_of') as Array<{ target_node_id: string; weight: number }>;
