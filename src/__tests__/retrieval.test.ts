...
describe("spreadingActivation", () => {
	...
	it("resolves name entry points via fuzzy fallback when exact match fails", () => {
		const db = freshDb();
		const named = makeNode(db, {
			name: "Switch from Express to Fastify",
			strength: 1.0,
		});
		const neighbor = makeNode(db, { name: "neighbor", strength: 1.0 });
	...
	it("prefers exact name match over fuzzy when both exist", () => {
		...
		const fuzzy = makeNode(db, {
			name: "Switch from Express to Fastify",
			strength: 1.0,
		});
		const exactNeighbor = makeNode(db, {
			name: "exact-neighbor",
			strength: 1.0,
		});
		const fuzzyNeighbor = makeNode(db, {
			name: "fuzzy-neighbor",
			strength: 1.0,
		});
