...
import type { RawToolCall, EngramEvent, FileReadEvent, FileWriteEvent, TestRunEvent } from '../types.js';
...
describe('event-stream', () => {
  describe('classifyToolCall', () => {
    ...
    it('classifies Edit as file_write', () => {
      const result = classifyToolCall(makeToolCall({
        tool_name: 'Edit',
        tool_input: { file_path: '/edit.ts', old_string: 'old', new_string: 'new\ncode' },
      }));
      expect(result).not.toBeNull();
      expect(result!.type).toBe('file_write');
  ...
  describe('detectDerivedEvents', () => {
    const baseEvent = (type: string, extra: Record<string, unknown> = {}): EngramEvent => ({
      type,
    ...
    it('returns exploration for file_read with no prior reads', () => {
      const event = baseEvent('file_read', { filePath: '/src/new.ts' }) as FileReadEvent;
    ...
    it('returns empty for file_read of already-read path', () => {
      const event = baseEvent('file_read', { filePath: '/src/old.ts' }) as FileReadEvent;
      const prior = [baseEvent('file_read', { filePath: '/src/old.ts' }) as FileReadEvent];
      const result = detectDerivedEvents(event, prior);
      expect(result).toEqual([]);
    });

    it('returns fix_attempt for file_write after failed test', () => {
      const event = baseEvent('file_write', { filePath: '/src/fix.ts', linesChanged: 5, evidenceSnippet: '' }) as FileWriteEvent;
      const prior: EngramEvent[] = [
        baseEvent('test_run', { command: 'npm test', exitCode: 1, passed: false }) as TestRunEvent,
      ];
      const result = detectDerivedEvents(event, prior);
    ...
    it('returns progression for file_write of new file after passed test', () => {
      const event = baseEvent('file_write', { filePath: '/src/brand-new.ts', linesChanged: 10, evidenceSnippet: '' }) as FileWriteEvent;
      const prior: EngramEvent[] = [
        baseEvent('test_run', { command: 'npm test', exitCode: 0, passed: true }) as TestRunEvent,
      ];
      const result = detectDerivedEvents(event, prior);
    ...
    it('does not return progression for file_write of already-written file after passed test', () => {
      const event = baseEvent('file_write', { filePath: '/src/existing.ts', linesChanged: 2, evidenceSnippet: '' }) as FileWriteEvent;
      const prior: EngramEvent[] = [
        baseEvent('test_run', { command: 'npm test', exitCode: 0, passed: true }) as TestRunEvent,
        baseEvent('file_write', { filePath: '/src/existing.ts', linesChanged: 3, evidenceSnippet: '' }) as FileWriteEvent,
      ];
      const result = detectDerivedEvents(event, prior);
    ...
    it('returns repeated_revision when 3rd write to same path', () => {
      const event = baseEvent('file_write', { filePath: '/src/churn.ts', linesChanged: 1, evidenceSnippet: '' }) as FileWriteEvent;
      const prior: EngramEvent[] = [
        baseEvent('file_write', { filePath: '/src/churn.ts', linesChanged: 1, evidenceSnippet: '' }) as FileWriteEvent,
        baseEvent('file_write', { filePath: '/src/churn.ts', linesChanged: 2, evidenceSnippet: '' }) as FileWriteEvent,
      ];
      const result = detectDerivedEvents(event, prior);
    ...
    it('returns expanding_search for 5 reads across distant directories', () => {
      ...
      const prior: EngramEvent[] = priorPaths.map(fp =>
        baseEvent('file_read', { filePath: fp }) as FileReadEvent,
      );
      const event = baseEvent('file_read', { filePath: '/d/e/f.ts' }) as FileReadEvent;
      const result = detectDerivedEvents(event, prior);
    ...
    it('does not return expanding_search for 5 reads in same directory', () => {
      ...
      const event = baseEvent('file_read', { filePath: '/src/e.ts' }) as FileReadEvent;
      const result = detectDerivedEvents(event, prior);
  ...
  describe('appendEvent + getSessionEvents', () => {
    ...
    it('round-trips 3 events in order', () => {
      ...
      const events: EngramEvent[] = [
        { type: 'file_read', sessionId: 's', timestamp: '2024-01-01T00:00:00Z', filePath: '/a.ts' },
        { type: 'file_read', sessionId: 's', timestamp: '2024-01-01T00:00:01Z', filePath: '/b.ts' },
        { type: 'build', sessionId: 's', timestamp: '2024-01-01T00:00:02Z', command: 'tsc', exitCode: 0 },
      ];
    ...
    it('does not throw on read-only directory path', () => {
      const dir = path.join(os.tmpdir(), `engram-readonly-${crypto.randomUUID()}`);
      fs.mkdirSync(dir, { recursive: true });
