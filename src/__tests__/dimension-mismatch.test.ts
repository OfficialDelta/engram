...
describe('validateEmbeddingDimension', () => {
  ...
  it('returns null when no metadata rows exist', () => {
    ...
    try {
      ...
      db.prepare("DELETE FROM metadata WHERE key = 'embedding_dimension'").run();
      const result = validateEmbeddingDimension(db, 'voyage-3');
      expect(result).toBeNull();
    } finally {
      db.close();
...
describe('initializeSchema dimension mismatch', () => {
  ...
  it('succeeds with rebuild when allowRebuild is true', () => {
    ...
    try {
      const dimRow = db2.prepare("SELECT value FROM metadata WHERE key = 'embedding_dimension'").get() as { value: string };
      expect(Number(dimRow.value)).toBe(1024);

      const provRow = db2.prepare("SELECT value FROM metadata WHERE key = 'embedding_provider'").get() as { value: string };
  ...
  it('works on first init with fresh DB', () => {
    ...
    const db = initializeSchema(dbPath, 512, 'voyage-3-lite');
    try {
      const dimRow = db.prepare("SELECT value FROM metadata WHERE key = 'embedding_dimension'").get() as { value: string };
      expect(Number(dimRow.value)).toBe(512);

      const provRow = db.prepare("SELECT value FROM metadata WHERE key = 'embedding_provider'").get() as { value: string };
      expect(provRow.value).toBe('voyage-3-lite');

      const verRow = db.prepare("SELECT value FROM metadata WHERE key = 'schema_version'").get() as { value: string };
