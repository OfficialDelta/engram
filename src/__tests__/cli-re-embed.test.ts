...
describe('runReEmbed', () => {
  ...
  it('rebuilds vec0 and updates metadata with 0 nodes', async () => {
    ...
    expect(result).toEqual({ nodesEmbedded: 0, provider: 'voyage-3-lite', dimension: 512 });
    expect(vi.mocked(getEmbedding)).not.toHaveBeenCalled();
    ...
    try {
      const dim = verifyDb.prepare("SELECT value FROM metadata WHERE key = 'embedding_dimension'").get() as { value: string };
      const prov = verifyDb.prepare("SELECT value FROM metadata WHERE key = 'embedding_provider'").get() as { value: string };
  ...
  it('embeds 5 nodes in a single batch', async () => {
    ...
    sqliteVec.load(verifyDb);
    try {
      const count = verifyDb.prepare('SELECT COUNT(*) as c FROM node_embeddings').get() as { c: number };
