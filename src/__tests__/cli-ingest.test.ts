...
vi.mock('../core/project-identity.js', async () => {
  const actual = await vi.importActual<typeof import('../core/project-identity.js')>('../core/project-identity.js');
  return {
    ...actual,
    getDbPath: vi.fn(),
  };
...
describe('runIngest', () => {
  ...
  it('creates file nodes for source files', async () => {
    ...
    sqliteVec.load(db);
    try {
      const rows = db.prepare("SELECT * FROM nodes WHERE node_type = 'file'").all() as Array<{ name: string }>;
  ...
  it('creates import edges between files', async () => {
    ...
    sqliteVec.load(db);
    try {
      const edges = db.prepare("SELECT * FROM edges WHERE relationship_type = 'imports'").all() as Array<{ source_node_id: string; target_node_id: string }>;
  ...
  it('creates documentation concept node for README', async () => {
    ...
    try {
      ...
      ).all() as Array<{ name: string; strength: number; description: string }>;
  ...
  it('creates git commit episodes', async () => {
    ...
    try {
      const rows = db.prepare(
        "SELECT * FROM episodes WHERE session_id LIKE 'ingest-git-%'"
      ).all();
  ...
  it('returns zeros when DB does not exist', async () => {
    vi.mocked(getDbPath).mockReturnValue(path.join(tmpDir, 'nonexistent', 'engram.db'));
  ...
  it('skips node_modules and .git directories', async () => {
    ...
    sqliteVec.load(db);
    try {
      const rows = db.prepare("SELECT name FROM nodes WHERE node_type = 'file'").all() as Array<{ name: string }>;
      for (const row of rows) {
  ...
  it('handles non-git directory gracefully', async () => {
    ...
    sqliteVec.load(db);
    try {
      const fileNodes = db.prepare("SELECT COUNT(*) as c FROM nodes WHERE node_type = 'file'").get() as { c: number };
      expect(fileNodes.c).toBeGreaterThanOrEqual(3);
      const episodes = db.prepare("SELECT COUNT(*) as c FROM episodes").get() as { c: number };
