...
describe('Ollama concurrent embedding via Promise.all', () => {
  ...
  it('fires all fetch calls concurrently and returns one embedding per text', async () => {
    ...
    const { getEmbedding } = await vi.importActual<typeof import('../core/embed.js')>('../core/embed.js');
    const result = await getEmbedding(['alpha', 'beta', 'gamma'], { provider: 'ollama' });
...
describe('applyGraphChanges batch embedding', () => {
  ...
  it('batches 5 create_new nodes into exactly 1 getEmbedding call', async () => {
    ...
    try {
      const changes: GraphChangeRequest = {
        nodesToCreate: [
          { name: 'Node-1', nodeType: 'concept', description: 'Desc 1', affectedFiles: [], causallyImportant: false },
          { name: 'Node-2', nodeType: 'pattern', description: 'Desc 2', affectedFiles: [], causallyImportant: false },
          { name: 'Node-3', nodeType: 'file', description: 'Desc 3', affectedFiles: ['/a.ts'], causallyImportant: true },
          { name: 'Node-4', nodeType: 'entity', description: 'Desc 4', affectedFiles: [], causallyImportant: false },
          { name: 'Node-5', nodeType: 'decision', description: 'Desc 5', affectedFiles: [], causallyImportant: true },
        ],
        nodesToUpdate: [],
        edgesToCreate: [],
      };
  ...
  it('batches only non-merge ops (3 create_new out of 5 total)', async () => {
    ...
    try {
      ...
      let callIdx = 0;
      vi.mocked(resolveEntity).mockImplementation(async (_db, name) => {
        callIdx++;
        if (callIdx <= 2) {
          return { action: 'merge', existingNodeId: callIdx === 1 ? existing1.id : existing2.id };
        }
      ...
      const changes: GraphChangeRequest = {
        nodesToCreate: [
          { name: 'Merge-1', nodeType: 'concept', description: 'Will merge', affectedFiles: [], causallyImportant: false },
          { name: 'Merge-2', nodeType: 'pattern', description: 'Will merge', affectedFiles: [], causallyImportant: false },
          { name: 'New-3', nodeType: 'file', description: 'Create new', affectedFiles: [], causallyImportant: false },
          { name: 'New-4', nodeType: 'entity', description: 'Create new', affectedFiles: [], causallyImportant: false },
          { name: 'New-5', nodeType: 'decision', description: 'Create new', affectedFiles: [], causallyImportant: false },
        ],
        nodesToUpdate: [],
        edgesToCreate: [],
      };
  ...
  it('does not call getEmbedding when all nodes are merges', async () => {
    ...
    try {
      ...
      const changes: GraphChangeRequest = {
        nodesToCreate: [
          { name: 'MergeOnly-1', nodeType: 'concept', description: 'Merge', affectedFiles: [], causallyImportant: false },
          { name: 'MergeOnly-2', nodeType: 'pattern', description: 'Merge', affectedFiles: [], causallyImportant: false },
        ],
        nodesToUpdate: [],
        edgesToCreate: [],
      };
