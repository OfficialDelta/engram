...
describe("loadConfig", () => {
	...
	it("partial config merges correctly with defaults", () => {
		fs.writeFileSync(
			configPath,
			JSON.stringify({ llm: { pass1Model: "fast" } }),
		);
		const config = loadConfig(configPath);
...
describe("validateConfig", () => {
	...
	it("rejects unknown embedding provider", () => {
		const result = validateConfig({
			embedding: { provider: "unknown-provider" },
		});
...
describe("validateConfig maintenance", () => {
	it("accepts valid maintenance config", () => {
		const result = validateConfig({
			maintenance: { decayThreshold: 0.05, decayFactor: 0.9 },
		});
		expect(result.valid).toBe(true);
	});
	...
	it("rejects non-numeric decayThreshold", () => {
		...
		expect(result.errors[0]).toMatch(
			/maintenance.decayThreshold must be a positive number/,
		);
	});
	...
	it("rejects decayThreshold >= 1", () => {
		...
		expect(result.errors).toEqual(
			expect.arrayContaining([
				expect.stringMatching(/decayThreshold must be less than 1/),
			]),
		);
	});
	...
	it("rejects decayFactor >= 1", () => {
		...
		expect(result.errors).toEqual(
			expect.arrayContaining([
				expect.stringMatching(/decayFactor must be less than 1/),
			]),
		);
	});
	...
	it("rejects zero decayFactor", () => {
		...
		expect(result.valid).toBe(false);
		expect(result.errors[0]).toMatch(
			/maintenance.decayFactor must be a positive number/,
		);
	});
});
...
describe("loadConfig with maintenance", () => {
	it("loads maintenance section from config file", () => {
		const fileConfig = {
			maintenance: { decayThreshold: 0.1, decayFactor: 0.85 },
		};
		fs.writeFileSync(configPath, JSON.stringify(fileConfig));
		const config = loadConfig(configPath);
	...
	it("saveConfig then loadConfig preserves all fields across all sections", () => {
		const full: EngramConfig = {
			llm: {
				...
				pass2Model: "claude-opus-4-20250514",
			},
			embedding: {
