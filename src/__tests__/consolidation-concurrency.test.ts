...
describe('acquireConsolidationLock', () => {
  it('succeeds on first call and creates lock file with pid and acquiredAt', () => {
    ...
    const lockPath = path.join(dataDir, 'episodes', 'sess-1.consolidating.lock');
    expect(fs.existsSync(lockPath)).toBe(true);

    const content = JSON.parse(fs.readFileSync(lockPath, 'utf-8')) as { pid: number; acquiredAt: string };
