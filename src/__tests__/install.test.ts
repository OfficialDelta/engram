...
vi.mock("../core/project-identity.js", () => ({
	...
	getDbPath: vi.fn((cwd: string) =>
		path.join(cwd, ".engram-data", "engram.db"),
	),
}));
...
describe("install CLI", () => {
	...
	it("hook commands contain absolute paths", () => {
		...
		for (const event of [
			...
			const entry = allEntries.find((h: { command: string }) =>
				h.command.includes("engram"),
			);
	...
	it("idempotent: running install twice does not duplicate hook entries", () => {
		...
		for (const event of [
			...
			const engramHooks = allEntries.filter((h: { command: string }) =>
				h.command.includes("engram"),
			);
	...
	it("install succeeds and config module is wired (getConfigPath accessible from install context)", () => {
		...
		expect(getConfigPath()).toBe(
			path.join(os.homedir(), ".engram", "config.json"),
		);
	});
});
